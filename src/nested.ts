...
export function sumPoints(questions: Question[]): number {
    let total = 0;
    questions.map(
        (question: Question): number => (total = total + question.points)
    );
    return total;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    questions.forEach((question: Question): void => {
        if (question.published) {
            publishedTotal = publishedTotal + question.points;
        }
    });
...
export function toCSV(questions: Question[]): string {
    let CSVHeaders = "id,name,options,points,published" + "\n";
    questions.map(
    ...
    CSVHeaders = CSVHeaders.slice(0, -1);
